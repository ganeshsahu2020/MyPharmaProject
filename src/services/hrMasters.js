...
export const loadMasters=async()=>{
  const [pl,sp,dp,emps]=await Promise.all([
    ...
    supabase.from('vw_user_management_ext').select('id,employee_id,first_name,last_name,plant_uid,subplant_uid,department_uid,status').eq('status','Active').order('employee_id',{ascending:true})
  ]);
  return {
    plants:pl.data||[],subplants:sp.data||[],departments:dp.data||[],
    employees:(emps.data||[]).map((e)=>({...e,label:`${e.employee_id} - ${e.first_name} ${e.last_name}`}))
  };
};

export const cascade={
  subplants:(all,plant)=>all.filter((s)=>!plant||s.plant_uid===plant),
  departments:(all,subplant)=>all.filter((d)=>!subplant||d.subplant_uid===subplant),
