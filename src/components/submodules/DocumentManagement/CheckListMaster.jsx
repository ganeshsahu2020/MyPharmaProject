...
const DATE_FIELDS = ['effective_date', 'date_of_receipt', 'inspection_date', 'submitted_at', 'approved_at'];
    ...
    <div className="inline-flex items-center justify-center gap-4">
      <label className="inline-flex items-center gap-1 text-sm">
        <input
          ...
          checked={value === true}
          onChange={() => {
            if (!disabled) onChange(value === true ? undefined : true);
          }}
          disabled={disabled}
        />
        ✔
      ...
      <label className="inline-flex items-center gap-1 text-sm">
        <input
          ...
          checked={value === false}
          onChange={() => {
            if (!disabled) onChange(value === false ? undefined : false);
          }}
          disabled={disabled}
        />
        ✖
      ...
      <div className="mt-6 mb-2 flex items-center gap-2">
        ...
        {canAdmin && !readOnly && (
          <div className="ml-auto flex gap-2">
            <button onClick={addColumn} className="border rounded px-2 py-1 text-xs">
              Add Column
            </button>
            <button onClick={addRow} className="border rounded px-2 py-1 text-xs">
              Add Row
            </button>
            {onHide && (
              <button onClick={onHide} className="border rounded px-2 py-1 text-xs text-red-600">
                <Trash2 size={12} /> Hide Section
              </button>
            )}
          </div>
        )}
                  ...
                  {canAdmin && !readOnly ? (
                    <div className="flex flex-col gap-1">
                      <input
                        ...
                        value={c.label || ''}
                        onChange={(e) => updateHeader(idx, { label: e.target.value })}
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => removeColumn(c.key)} className="text-red-600 text-xs">
                          Delete
                        </button>
                      </div>
                      <input
                        className="border rounded px-2 py-1 w-full text-xs text-center"
                        value={c.placeholder || ''}
                        onChange={(e) => updateHeader(idx, { placeholder: e.target.value })}
                        placeholder="Placeholder (shown in cells)"
                      />
                    </div>
                  ) : (
                    <span>{c.label}</span>
              ...
              <tr key={`row-${ridx}`}>
                {(columns || []).map((c) => (
                  <td key={`${ridx}-${c.key}`} className="border p-2 text-center">
                    {c.type === 'checkbox' ? (
                      <>
                        <TwoBoxToggle value={r[c.key]} onChange={(val) => setCheckbox(ridx, c.key, val)} disabled={readOnly} />
                        {r[`${c.key}__audit`] && (
                          <div className="text-[10px] text-gray-500 mt-1">
                            {r[`${c.key}__audit`].value || ''} by {r[`${c.key}__audit`].by || '-'} on{' '}
                            {fmtDT(r[`${c.key}__audit`].at)}
                          </div>
                        )}
                      </>
                    ) : c.type === 'textarea' ? (
                ...
                <td className="border p-2 text-center">
                  {!readOnly && <button onClick={() => removeRow(ridx)} className="text-red-600 text-xs">Delete</button>}
                </td>
              </tr>
            ))}
            {(rows || []).length === 0 && (
              <tr>
                <td colSpan={((columns || []).length || 1) + 1} className="text-center text-gray-500 p-3 text-sm">
                  No rows yet
                </td>
              </tr>
            )}
  ...
  return (
    <div className="border rounded-md p-3 mt-4">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          <button className="border rounded px-2 py-1 text-xs" onClick={addOne} disabled={disabled}>
            +1
          </button>
          <button className="border rounded px-2 py-1 text-xs" onClick={add16Blanks} disabled={disabled}>
            +16 blanks
          </button>
          <button
            className="border rounded px-2 py-1 text-xs"
            onClick={() => applyToRows('append')}
            disabled={disabled || !(placeholders || []).length}
          >
            Append → Rows
          </button>
          <button
            className="border rounded px-2 py-1 text-xs"
            onClick={() => applyToRows('replace')}
            disabled={disabled || !(placeholders || []).length}
          >
            Replace Rows
          </button>
      ...
      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {(placeholders || []).map((p, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <input
                ...
                disabled={disabled}
              />
              <button className="text-red-600 text-xs" onClick={() => removeAt(idx)} disabled={disabled}>
                Delete
              </button>
            </div>
          ))}
        ...
        <div>
          ...
          <textarea
            className="border rounded w-full px-2 py-2 min-h-[120px]"
            value={bulk}
            onChange={(e) => setBulk(e.target.value)}
            placeholder="Enter one placeholder per line"
            disabled={disabled}
          />
          <div className="mt-2">
            <button className="border rounded px-2 py-1 text-xs" onClick={bulkAdd} disabled={disabled}>
              Bulk Add
            </button>
          </div>
        </div>
...
const ChecklistPreview = ({ form, headerMeta, showHdr }) => (
  <div className="print-a4 p-6 text-sm leading-6 bg-white text-black">
    ...
    <div className="print-header mb-3 flex items-center gap-3">
      ...
      <div className="ml-auto text-xs text-gray-600">Printed: {fmtDT(new Date().toISOString())}</div>
    </div>
    ...
    <div className="grid grid-cols-2 gap-2 mb-4">
      ...
      {headerMeta.show_date_of_receipt !== false && (
        <div>Date of Receipt: {showHdr(form.date_of_receipt, 'date_of_receipt')}</div>
      )}
      {headerMeta.show_quantity_received !== false && (
        <div>Quantity Received: {showHdr(form.quantity_received, 'quantity_received')}</div>
      )}
      {headerMeta.show_storage_condition !== false && (
        <div>Storage Condition: {showHdr(form.storage_condition, 'storage_condition')}</div>
      )}
      {headerMeta.show_inspection_date !== false && (
        <div>Inspection Date: {showHdr(form.inspection_date, 'inspection_date')}</div>
      )}
...
const TemplateLibrary = ({ form, setForm, canAdmin, readOnly }) => {
  ...
  const loadTemplates = async ({ all = false } = {}) => {
    ...
    try {
      if (!all) {
        ...
        // Tolerant: select('*') to avoid 400s on missing/renamed columns
        const res = await supabase.from('checklist_template').select('*').order('name', { ascending: true });
        data = res.data;
        error = res.error;
      }
    } catch (err) {
  ...
  const applyPlaceholders = (section, mode) => {
    ...
    const list =
      section === 'visual' ? tpl.visual_placeholders : section === 'doc' ? tpl.doc_placeholders : tpl.test_placeholders;
    if (!Array.isArray(list) || list.length === 0) {
      toast.error('Template has no placeholders in this section');
      return;
    }
    const cols = form[section + '_cols'] || [];
    const key = firstRequiredKey(cols);
    if (!key) {
      toast.error('Add a column or mark one as required first.');
      return;
    }
    const makeBlankRow = () => {
      const o = {};
      cols.forEach((c) => {
        o[c.key] = c.type === 'checkbox' ? undefined : '';
      });
      return o;
    };
    const buildRows = (arr) =>
  ...
  const applyColsTitles = (section) => {
    ...
    if (!canAdmin) {
      toast.error('Only Admin can replace columns/titles');
      return;
    }
    if (window.confirm('Replace columns & title (and header placeholders if present) with the template values?') !== true)
      return;
    ...
    if (section === 'visual') {
      setForm((f) => ({
        ...f,
        visual_cols: tpl.visual_cols || [],
        visual_title: tpl.visual_title || f.visual_title,
        header_meta: { ...DEFAULT_HEADER_META(), ...(f.header_meta || {}), ...header_meta },
      }));
    } else if (section === 'doc') {
      setForm((f) => ({
        ...f,
        doc_cols: tpl.doc_cols || [],
        doc_title: tpl.doc_title || f.doc_title,
        header_meta: { ...DEFAULT_HEADER_META(), ...(f.header_meta || {}), ...header_meta },
      }));
    } else {
      setForm((f) => ({
        ...f,
        test_cols: tpl.test_cols || [],
        test_title: tpl.test_title || f.test_title,
        header_meta: { ...DEFAULT_HEADER_META(), ...(f.header_meta || {}), ...header_meta },
      }));
    }
  ...
  const computeScopeForSave = () => {
    let scope = saveScope;
    if (scope === 'auto') {
      scope = submoduleId ? 'submodule' : moduleId ? 'module' : 'global';
    }
    return {
      module_id: scope === 'module' || scope === 'submodule' ? moduleId : null,
      submodule_id: scope === 'submodule' ? submoduleId : null,
      scope,
    };
  };
  ...
  const saveAsNew = async () => {
    if (!canAdmin) {
      toast.error('Only Admin can save templates');
      return;
    }
    const name = window.prompt('Template name?', 'My Checklist Template');
    if (!name) return;

    const { module_id, submodule_id, scope } = computeScopeForSave();
    if (scope !== 'global' && !module_id) {
      toast.error('Pick a Linked Module/Submodule first or set scope to Global.');
      return;
    }
  ...
  const updateSelected = async () => {
    ...
    if (window.confirm(`Update "${tpl.name}" with current screen values?`) !== true) return;
  ...
  const markApprovedAndLock = async () => {
    ...
    if (!canAdmin) {
      toast.error('Only Admin can approve/lock');
      return;
    }
    try {
      ...
      toast.error(
        'Your template table has no status/locked/version columns. Rename template to include "[Approved]" to prefer it automatically.'
      );
    }
  };
    ...
    <div className="mt-6 border rounded-md p-3">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button className="border rounded px-2 py-1 text-xs" onClick={() => loadTemplates({ all: false })}>
            Refresh Scoped
          </button>
          <button className="border rounded px-2 py-1 text-xs" onClick={() => loadTemplates({ all: true })}>
            Show All
          </button>
          <button className="border rounded px-2 py-1 text-xs" onClick={saveAsNew} disabled={readOnly || !canAdmin}>
            <Upload size={14} /> Save as New
          </button>
          <button
            className="border rounded px-2 py-1 text-xs"
            onClick={updateSelected}
            disabled={readOnly || !canAdmin || !selected}
          >
            Update Selected
          </button>
          <button
            className="border rounded px-2 py-1 text-xs inline-flex items-center gap-1"
            onClick={markApprovedAndLock}
            disabled={!canAdmin || !selected}
          >
      ...
      <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
        <label className="flex flex-col gap-1">
          ...
          <select className="border rounded px-3 py-2" value={selected} onChange={(e) => setSelected(e.target.value)}>
            ...
            {templates.map((t) => {
              ...
              const approved =
                t.status === 'Approved' || t.locked || (t.name || '').toLowerCase().includes('approved');
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              className="border rounded px-2 py-1 text-xs"
              onClick={() => applyPlaceholders('visual', 'append')}
            ...
            <button
              className="border rounded px-2 py-1 text-xs"
              onClick={() => applyPlaceholders('visual', 'replace')}
              disabled={!selected || readOnly}
            >
            ...
            <button
              className="border rounded px-2 py-1 text-xs"
              onClick={() => applyColsTitles('visual')}
        ...
        <div className="md:col-span-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              className="border rounded px-2 py-1 text-xs"
              onClick={() => applyPlaceholders('test', 'append')}
              disabled={!selected || readOnly}
            >
              Append Placeholders → Rows
            </button>
            <button
              className="border rounded px-2 py-1 text-xs"
              onClick={() => applyPlaceholders('test', 'replace')}
              disabled={!selected || readOnly}
            >
              Replace Rows from Placeholders
            </button>
...
const CheckListMaster = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (workflowFilter === 'Draft') query = query.eq('workflow_status', 'Draft');
      else if (workflowFilter === 'Approved') query = query.eq('workflow_status', 'Approved');
      else if (workflowFilter === 'Rejected') query = query.eq('workflow_status', 'Rejected');
      else if (workflowFilter === 'Submitted') query = query.eq('workflow_status', 'Submitted');
      else if (workflowFilter === 'Reviewed') query = query.eq('workflow_status', 'Reviewed');
  ...
  const applyFullTemplate = (tpl) => {
    ...
    const buildRows = (ph, cols) => {
      ...
      return ph.map((p) => {
        const row = {};
        (cols || []).forEach((c) => {
          row[c.key] = c.type === 'checkbox' ? undefined : '';
        });
        row[key] = p;
        return row;
      });
  ...
  const tryAutoApplyTemplate = async () => {
    ...
    const best =
      list.find(
        (t) => t.status === 'Approved' || t.locked === true || (t.name || '').toLowerCase().includes('approved')
      ) || list[0];
  ...
  }, [form.linked_module, form.linked_submodule, autoApplyOnLink]);
  ...
  const handleDelete = async (id) => {
    const canOwnerDelete = form.created_by === email && form.workflow_status !== 'Approved';
    if (!(canAdmin || canOwnerDelete)) {
      toast.error('You cannot delete this checklist.');
      return;
    }
    ...
    if (error) {
      toast.error(error.message);
      bumpStatus({ lastOp: 'delete-error', lastError: error.message });
    } else {
      toast.success('Deleted');
      bumpStatus({ lastOp: 'delete-ok', lastError: '', lastSaveId: id });
      fetchAll();
      if (editingId === id) {
        newChecklist();
      }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        const { _drop, ...payloadUpdate } = payloadBase;
        const ownerOrAny = !['Approved', 'Rejected'].includes(form.workflow_status);
        if (!canAdmin && !ownerOrAny) {
          toast.error('Locked: only Admin can edit at this stage.');
          setSaving(false);
          return null;
        }
        res = await supabase.from('checklist_master').update(payloadUpdate).eq('id', editingId).select().single();
      } else {
        const { _drop, ...payloadInsert } = payloadBase;
        res = await supabase.from('checklist_master').insert([payloadInsert]).select().single();
      }
    } catch (err) {
  ...
  const approveViaRPC = async (id, status, reason = '') => {
    ...
    if (error) {
      ...
      const hmCurrent = { ...DEFAULT_HEADER_META(), ...(form.header_meta || {}) };
      const hmPatch = { ...hmCurrent };
      const patch = { workflow_status: status };

      if (status === 'Reviewed') {
        patch.reviewed_by = form.reviewed_by || email;
        hmPatch.reviewed_signed_at = nowISO;
      }
      if (status === 'Approved') {
        patch.approved_by_user = email;
        patch.approved_at = nowISO;
        patch.approved_by = form.approved_by || email;
        hmPatch.approved_signed_at = nowISO;
      }
      if (status === 'Rejected') {
        patch.approved_by_user = email;
        patch.approved_at = nowISO;
        hmPatch.approved_signed_at = nowISO;
        patch.remarks = reason ? `${form.remarks || ''}\n[Decision]: Rejected (${reason})` : form.remarks;
      }

      const upd = await supabase
        .from('checklist_master')
        .update({ ...patch, header_meta: hmPatch })
        .eq('id', id)
        .select()
        .single();
      if (upd.error) {
        toast.error(upd.error.message);
        bumpStatus({ lastOp: 'decision-fallback-error', lastError: upd.error.message });
        return;
      }
      ...
      setForm({
        ...upd.data,
        checklist_title: upd.data?.checklist_title || merged.checklist_name_value || '',
        header_meta: merged,
      });
      setRows((prev) => prev.map((r) => (r.id === id ? upd.data : r)));
      bumpStatus({ lastOp: `${status}-fallback-ok`, lastError: '' });
      if (status === 'Approved') {
        await trySetActive(id, true);
      }
      return;
    }
    ...
    setForm({
      ...data,
      checklist_title: data?.checklist_title || hmOut.checklist_name_value || '',
      header_meta: hmOut,
    });
    setRows((prev) => prev.map((r) => (r.id === id ? data : r)));
    bumpStatus({ lastOp: `${status}-ok`, lastError: '' });
    setEditingId(id);
    if (status === 'Approved') {
      await trySetActive(id, true);
    }
  ...
  const submitViaRPC = async (id) => {
    ...
    if (error) {
      const upd = await supabase
        .from('checklist_master')
        .update({ workflow_status: 'Submitted', submitted_by: email, submitted_at: new Date().toISOString() })
        .eq('id', id)
        .select()
        .single();
      if (upd.error) {
  ...
  const handleSubmitButton = async () => {
    if (form.workflow_status !== 'Draft') {
      toast('Only Drafts can be submitted');
      return;
    }
    let id = editingId;
    if (!id) {
  ...
  const trySetActive = async (id, val) => {
    const attempt = async (fields) => supabase.from('checklist_master').update(fields).eq('id', id).select().single();
    let upd = await attempt({ is_active: val });
    if (upd.error) upd = await attempt({ active: val });
    if (upd.error) upd = await attempt({ status: val ? 'Active' : 'Inactive' });
    if (upd.error) {
      toast.error(upd.error.message);
      bumpStatus({ lastOp: 'activate-error', lastError: upd.error.message });
      return null;
    }
    const d = upd.data;
    const hmOut = { ...DEFAULT_HEADER_META(), ...(d?.header_meta || {}) };
    setForm({
      ...d,
      checklist_title: d?.checklist_title || hmOut.checklist_name_value || '',
      header_meta: hmOut,
      print_with_placeholders: d?.print_with_placeholders ?? true,
    });
    setRows((prev) => prev.map((r) => (r.id === id ? d : r)));
    bumpStatus({ lastOp: 'activate-ok', lastError: '' });
    toast.success(`Set ${val ? 'Active' : 'Inactive'} for linking`);
    return true;
  };

  const reopenToDraft = async (id) => {
    const upd = await supabase
      .from('checklist_master')
      .update({
        workflow_status: 'Draft',
    ...
    if (upd.error) {
      toast.error(upd.error.message);
      bumpStatus({ lastOp: 'reopen-error', lastError: upd.error.message });
      return;
    }
    const d = upd.data;
    const hmOut = { ...DEFAULT_HEADER_META(), ...(d?.header_meta || {}) };
    ...
    setRows((prev) => prev.map((r) => (r.id === id ? d : r)));
    bumpStatus({ lastOp: 'reopen-ok', lastError: '' });
    toast.success('Reopened to Draft');
  };

  const quickSetWorkflow = async (id, nextStatus) => {
    if (!id) return;
    if (nextStatus === 'Submitted') {
      await submitViaRPC(id);
    } else if (nextStatus === 'Reviewed') {
      await approveViaRPC(id, 'Reviewed', '');
    } else if (nextStatus === 'Approved') {
      await approveViaRPC(id, 'Approved', '');
    } else if (nextStatus === 'Rejected') {
      const reason = window.prompt('Reason for rejection (optional)', '') || '';
      await approveViaRPC(id, 'Rejected', reason);
    } else if (nextStatus === 'Draft') {
      await reopenToDraft(id);
      await trySetActive(id, false);
    }
  };

  const setActiveFlagFromUI = async (val) => {
    let id = editingId;
    if (!id) {
      id = await handleSave();
      if (!id) return;
    }
    await trySetActive(id, !!val);
  };
  ...
  const setVisualRows = (next) =>
    setForm((f) => ({ ...f, visual_inspection: typeof next === 'function' ? next(f.visual_inspection) : next }));
  const setDocRows = (next) =>
    setForm((f) => ({ ...f, documentation_review: typeof next === 'function' ? next(f.documentation_review) : next }));
  const setTestRows = (next) =>
    setForm((f) => ({ ...f, physical_tests: typeof next === 'function' ? next(f.physical_tests) : next }));
  ...
  const applyLocalPlaceholders = (section, mode) => {
    ...
    if (!Array.isArray(list) || list.length === 0) {
      toast.error('No placeholders to apply');
      return;
    }
    const makeBlankRow = () => {
      const o = {};
  ...
  const resetHeaderPlaceholdersFromTemplate = async () => {
    ...
    const tpl = (data || [])[0];
    if (!tpl || !tpl.header_meta) {
      toast.error('No scoped template or template has no header placeholders');
      return;
    }
  ...
  // IMPORTANT: anyone can edit while Draft/Submitted/Reviewed; locked on Approved/Rejected
  const disableSavedEdit = !canAdmin && !!editingId && ['Approved', 'Rejected'].includes(form.workflow_status);

  const submodsForModule = useMemo(
    () => submodules.filter((s) => !form.linked_module || s.module_id === form.linked_module),
    [submodules, form.linked_module]
  );
  ...
  const performSearch = async () => {
    ...
    setSearching(true);
    try {
      // Tolerant select('*') to avoid 400s if some columns (e.g., status) don't exist
      let qry = supabase
        .from('checklist_master')
        .select('*')
        .or(`checklist_title.ilike.%${q}%,version.ilike.%${q}%,sop_reference.ilike.%${q}%`)
        .order('created_at', { ascending: false })
        .limit(50);
      if (ownerFilter === 'mine') qry = qry.eq('created_by', email);
      if (workflowFilter === 'Draft') qry = qry.eq('workflow_status', 'Draft');
      else if (workflowFilter === 'Approved') qry = qry.eq('workflow_status', 'Approved');
      else if (workflowFilter === 'Rejected') qry = qry.eq('workflow_status', 'Rejected');
      else if (workflowFilter === 'Submitted') qry = qry.eq('workflow_status', 'Submitted');
      else if (workflowFilter === 'Reviewed') qry = qry.eq('workflow_status', 'Reviewed');

      const { data, error } = await qry;
      if (error) {
        toast.error(error.message);
        bumpStatus({ lastOp: 'search-error', lastError: error.message });
        setSearchResults([]);
      } else {
        setSearchResults(data || []);
        bumpStatus({ lastOp: 'search-ok', lastError: '', lastFetchCount: (data || []).length });
      }
    } catch (err) {
  ...
  const stampSignature = (who) => {
    ...
    setForm((f) => ({
      ...f,
      [who + '_by']: f[who + '_by'] || undefined,
      header_meta: { ...(f.header_meta || {}), [key]: new Date().toISOString() },
  ...
  const currentActiveFlag = (() => {
    if (form?.is_active !== undefined) return !!form.is_active;
    if (form?.active !== undefined) return !!form.active;
    const s = String(form?.status ?? 'Active').toLowerCase();
    return s !== 'inactive';
  })();
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-3 text-xs flex items-center gap-2 text-gray-700">
        ...
        {status.lastSaveId && (
          <span>
            lastId: <b>{status.lastSaveId}</b>
          </span>
        )}
        {status.lastFetchCount != null && (
          <span>
            rows: <b>{status.lastFetchCount}</b>
          </span>
        )}
      ...
      {issues.length > 0 && (
        <div className="mb-4 border-l-4 border-amber-500 bg-amber-50 p-3 text-sm">
          <div className="flex items-center gap-2 font-semibold">
            <AlertTriangle size={16} /> Readiness & Missing Content
          </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <ClipboardList /> Check List Master
        </h1>
        <div className="flex items-center gap-2">
          <button onClick={newChecklist} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">
            <Plus size={16} /> New
          </button>
          <button onClick={resetBodyToDefaults} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">
            <RotateCcw size={16} /> Reset Sections
          </button>
          <label className="text-xs flex items-center gap-1 border rounded px-2 py-1">
            <input
              type="checkbox"
              checked={autoApplyOnLink}
              onChange={(e) => setAutoApplyOnLink(e.target.checked)}
            />
          ...
          <label className="text-xs flex items-center gap-1 border rounded px-2 py-1">
            <input
              type="checkbox"
              checked={!!form.print_with_placeholders}
              onChange={(e) => setForm((f) => ({ ...f, print_with_placeholders: e.target.checked }))}
              disabled={disableSavedEdit}
            />
            Print: use placeholders when empty
          </label>
          {(canAdmin || !editingId || !['Approved', 'Rejected'].includes(form.workflow_status)) && (
            <button
              onClick={handleSave}
              disabled={saving}
              className={cls(
          ...
          <button onClick={handlePrint} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">
      ...
      <div className="mt-4 border rounded-md p-3">
        <div className="flex items-center">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button className="border rounded px-2 py-1 text-xs" onClick={() => setAllPredef(true)} disabled={disableSavedEdit}>
              Select All
            </button>
            <button className="border rounded px-2 py-1 text-xs" onClick={() => setAllPredef(false)} disabled={disableSavedEdit}>
              Select None
            </button>
          </div>
        </div>
            ...
            return (
              <label key={p.key} className="text-sm flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={checked}
                  onChange={(e) => setPredef(p.key, e.target.checked)}
                  disabled={disableSavedEdit}
                />
                {p.label}
              </label>
        ...
        <div className="text-xs text-gray-500 mt-2">
          Toggled fields are hidden from the header and printout but remain in history if previously filled.
        </div>
      </div>
        ...
        <div className="mt-3">
          ...
          {showHeaderHints && (
            <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 border rounded p-3">
              {Object.keys(DEFAULT_HEADER_META())
                .filter((k) => !k.endsWith('_value'))
                .map((k) => (
                  <label key={k} className="flex flex-col gap-1">
                    <span className="text-xs text-gray-600">
                      {k} {k.includes('signed_at') ? '(ISO timestamp, auto-filled by Sign)' : 'placeholder'}
                    </span>
                    <input
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        ...
        <label className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            <input
              className={cls('border rounded-md px-3 py-2 flex-1', disableSavedEdit && 'bg-gray-100')}
            ...
            />
            {!disableSavedEdit && (
              <button className="border rounded px-2 text-xs" onClick={() => stampSignature('prepared')}>
                Sign
              </button>
            )}
          </div>
        ...
        <label className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            ...
            />
            {!disableSavedEdit && (
              <button className="border rounded px-2 text-xs" onClick={() => stampSignature('reviewed')}>
                Sign
              </button>
            )}
          </div>
        ...
        <label className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            ...
            />
            {!disableSavedEdit && (
              <button className="border rounded px-2 text-xs" onClick={() => stampSignature('approved')}>
                Sign
              </button>
            )}
          </div>
        ...
        <label className="flex flex-col gap-1">
          ...
          >
            <option value="">— Select —</option>
            {submodsForModule.map((s) => (
              <option key={s.submodule_id} value={s.submodule_id}>
                {s.submodule_name}
              </option>
            ))}
          </select>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="flex flex-col gap-1">
          ...
          <input
            className={cls('border rounded-md px-3 py-2', (!headerMeta.show_visual || disableSavedEdit) && 'bg-gray-100')}
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            className={cls('border rounded-md px-3 py-2', (!headerMeta.show_doc || disableSavedEdit) && 'bg-gray-100')}
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            className={cls('border rounded-md px-3 py-2', (!headerMeta.show_test || disableSavedEdit) && 'bg-gray-100')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        ...
        <div className="md:col-span-2">
          <label className="flex flex-col gap-1">
            ...
            <textarea
              className={cls('border rounded-md px-3 py-2 min-h[80px]', disableSavedEdit && 'bg-gray-100')}
              value={form.remarks || ''}
      ...
      {/* Workflow controls + Active toggle */}
      {editingId && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span className="text-sm">
            Workflow: <b>{form.workflow_status || 'Draft'}</b>
          </span>

          {form.workflow_status === 'Draft' && (
            <button onClick={() => submitViaRPC(editingId)} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">
              <Send size={16} /> Submit for Review
            </button>
          )}

          {form.workflow_status === 'Submitted' && (
            <>
              <button
                onClick={() => approveViaRPC(editingId, 'Reviewed', '')}
                className="inline-flex items-center gap-2 px-3 py-2 rounded-md border text-indigo-700"
              >
              ...
              {canAdmin && (
                <>
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-3 py-2 rounded-md border text-red-700"
                  >
                    <XCircle size={16} /> Reject
                  </button>
                  <button onClick={() => reopenToDraft(editingId)} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">
                    Reopen to Draft
                  </button>
                </>
              )}
          ...
          {form.workflow_status === 'Reviewed' && (
            <>
              <button
                onClick={() => approveViaRPC(editingId, 'Approved', '')}
                className="inline-flex items-center gap-2 px-3 py-2 rounded-md border text-green-700"
              >
          ...
          {canAdmin && (
            <label className="text-xs flex items-center gap-1 ml-2">
              Active for linking
              <input type="checkbox" checked={currentActiveFlag} onChange={(e) => setActiveFlagFromUI(e.target.checked)} />
            </label>
          )}
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                setOwnerFilter('mine');
                setWorkflowFilter('Submitted');
              }}
            >
              My Submissions
            </button>
            <button className="border rounded px-2 py-1 text-xs inline-flex items-center gap-1" onClick={fetchAll}>
        ...
        <div className="mt-4 border-t pt-3">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder="Search by title, version, or SOP reference"
              value={q}
            ...
            <div className="mt-3 overflow-auto">
              <table className="min-w-[900px] w-full border rounded-md">
                ...
                <tbody>
                  {searchResults.map((r) => (
                    <tr key={r.id}>
                      <td className="border p-2">{r.checklist_title || r.header_meta?.checklist_name_value || '-'}</td>
                      <td className="border p-2">{r.version || '-'}</td>
                      <td className="border p-2">
                        {moduleName(r.linked_module)} / {submoduleName(r.linked_submodule)}
                      </td>
                      ...
                      <td className="border p-2">
                        <div className="text-xs leading-5">
                          <div>
                            <span className="text-gray-500">P:</span> {r.prepared_by || '-'}
                          </div>
                          <div>
                            <span className="text-gray-500">R:</span> {r.reviewed_by || '-'}
                          </div>
                          <div>
                            <span className="text-gray-500">A:</span> {r.approved_by || '-'}
                          </div>
                        </div>
                      </td>
                      <td className="border p-2">
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            onClick={async () => {
                              ...
                              if (error) {
                                toast.error(error.message);
                                bumpStatus({ lastOp: 'search-load-error', lastError: error.message });
                                return;
                              }
                              handleEdit(data);
                            }}
                          ...
                          {(canAdmin || (r.created_by === email && r.workflow_status !== 'Approved')) && (
                            <button className="text-red-600 inline-flex items-center gap-1" onClick={() => handleDelete(r.id)}>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td className="border p-2">
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleEdit(r)} className="text-blue-600 inline-flex items-center gap-1">
                            <Edit3 size={16} /> Edit
                          </button>
                          {canAdmin || (r.created_by === email && r.workflow_status !== 'Approved') ? (
                            <button onClick={() => handleDelete(r.id)} className="text-red-600 inline-flex items-center gap-1">
                              <Trash2 size={16} /> Delete
                            </button>
                          ) : (
                            <button onClick={handlePrint} className="text-gray-700 inline-flex items-center gap-1" title="Print">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-auto">
            <div className="sticky top-0 bg-white border-b p-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handlePrint} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md border" title="Print">
                  <Printer size={16} /> Print
                </button>
                <button
      ...
      <div className="mt-6 text-xs text-gray-500 leading-5">
        ...
        <ul className="list-disc ml-5">
          <li>
            Workflow is staged: <b>Draft → Submitted → Reviewed → Approved</b>. Review/Approve are now available to all
            users at the correct step. (Reject/Reopen remain admin-only.)
          </li>
          <li>Editing is open to all users until the record is Approved/Rejected. Admins can edit anytime.</li>
          <li>Delete is available to Admins anytime and to Owners until the record is Approved.</li>
        </ul>
      </div>
    </div>
  );
