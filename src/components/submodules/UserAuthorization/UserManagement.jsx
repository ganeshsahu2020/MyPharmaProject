...
const IconInput = ({
  ...
  <div className="relative">
    ...
    <input
      ...
      className={`border rounded text-sm w-full p-2 ${rightSlot ? 'pl-8 pr-14' : 'pl-8'}`}
    />
  ...
  <div className="relative">
    ...
    <select
      ...
      className={`border rounded text-sm w-full p-2 pl-8 ${
        multiple ? 'pr-2 min-h-24' : 'pr-8'
      } appearance-none disabled:bg-gray-100 ${className}`}
    >
      {children}
    </select>
    {!multiple && (
...
export default function UserManagement() {
  ...
  async function loadAll() {
    ...
    await toast.promise(op, {
      ...
      success: 'Loaded',
      error: (e) => `Load failed: ${errText(e)}`,
    });
  }

  const formSubplants = useMemo(
    () => (form.plant_uid ? subplants.filter((s) => s.plant_uid === form.plant_uid) : []),
    [form.plant_uid, subplants]
  );
  const formDepartments = useMemo(
    () => (form.subplant_uid ? departments.filter((d) => d.subplant_uid === form.subplant_uid) : []),
    [form.subplant_uid, departments]
  );
  const tableSubplants = useMemo(
    () => (plantFilter ? subplants.filter((s) => s.plant_uid === plantFilter) : []),
    [plantFilter, subplants]
  );
  const tableDepartments = useMemo(
    () => (subplantFilter ? departments.filter((d) => d.subplant_uid === subplantFilter) : []),
    [subplantFilter, departments]
  );
  ...
  function handleEdit(u) {
    let plant_uid = u.plant_uid,
      subplant_uid = u.subplant_uid,
    ...
    const roles = Array.isArray(u.role_array)
      ? u.role_array
  ...
  async function handleDelete(id) {
    ...
    const op = supabase.rpc('um_delete_id', { p_id: id }).then(({ error }) => {
  ...
  function handlePwdCheck() {
    if (form.password.length === 0 && form.confirm_password.length === 0) {
  ...
  async function handleSave(e) {
    ...
    if (!form.email || !form.employee_id || !form.first_name) {
      toast.error('Employee ID, Email, and First Name are required');
      return;
    }
    ...
    try {
      if (!form.id && autoCreateAuth) {
        await toast.promise(createViaEdgeFunction(), {
          loading: 'Creating user & Auth...',
          success: 'User & Auth linked',
          error: (e) => errText(e),
        });
      } else {
        await toast.promise(saveViaRpc(form.password || null), {
          loading: form.id ? 'Updating user...' : 'Saving user...',
          success: form.id ? 'User updated' : 'User added',
          error: (e) => errText(e),
        });
      }
      resetForm();
        ...
        >
          ...
          <div>
            ...
            <IconSelect
              ...
              onChange={(e) => setForm((f) => ({ ...f, department_uid: e.target.value }))}
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <IconSelect
              icon={Shield}
              value={form.role_array}
              onChange={(e) => {
                const vals = [...e.target.selectedOptions].map((o) => o.value);
                setForm((f) => ({ ...f, role_array: vals }));
              }}
              leftColor="text-amber-600"
              multiple
              className="pl-8"
            >
              {ROLE_OPTIONS.map((r) => (
          ...
          {!form.id && (
            <div className="flex items-center gap-2 col-span-full">
              ...
              <label htmlFor="autoAuth" className="text-xs">
                Auto-create Auth account & link
              </label>
            </div>
          )}
        ...
        <div className="mb-6 bg-white border rounded">
          <div className="p-3">
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
              ...
              <IconSelect
                icon={Building2}
                value={subplantFilter}
                onChange={(e) => {
                  setSubplantFilter(e.target.value);
                  setDepartmentFilter('');
                }}
                leftColor="text-green-600"
