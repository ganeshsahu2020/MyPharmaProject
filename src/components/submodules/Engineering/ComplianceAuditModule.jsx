// src/components/submodules/Engineering/ComplianceAuditModule.jsx
...
const ComplianceAuditModule = () => {
  ...
  const upsert = async () => {
    try {
      ...
      setEditing(false);
      setForm({ id: null, dev_code: '', title: '', description: '', status: 'Open' });
      fetchAll();
    } catch (e) {
      alert(e.message);
    }
                ...
                <tr key={r.id} className="border-t">
                  ...
                  <td className="p-2">
                    <Button variant="ghost" onClick={() => {
                      setForm({ id: r.id, dev_code: r.dev_code, title: r.title, description: r.description || '', status: r.status });
                      setEditing(true);
                    }}>
                      <Edit3 size={16} />
                    </Button>
          ...
          <div className="bg-white rounded-xl p-4 w-full max-w-xl shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                placeholder="Deviation Code"
                value={form.dev_code}
                onChange={(e) => setForm({ ...form, dev_code: e.target.value })}
              />
              <select
                className="border p-2 rounded"
                value={form.status}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setEditing(false); setForm({ id: null, dev_code: '', title: '', description: '', status: 'Open' }); }}>
                Cancel
              </Button>
