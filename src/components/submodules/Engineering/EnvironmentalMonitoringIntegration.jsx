...
const EnvironmentalMonitoringIntegration = () => {
  ...
  const fetchAll = async () => {
    ...
    if (!a.error) {
      const map = {};
      (a.data || []).forEach((x) => { map[x.tag_uid] = x.status; });
      setAlarms(map);
    }
  ...
  const acknowledge = (tag_uid) => toast.promise((async () => {
    ...
    const a = await supabase.from('ems_alarm').select('tag_uid,status').in('status', ['Open', 'Acknowledged']);
    if (!a.error) {
      const map = {}; (a.data || []).forEach((x) => { map[x.tag_uid] = x.status; });
      setAlarms(map);
    }
    await fetchAll();
  ...
  const IconSelect = ({ icon: Icon, label, value, onChange, children, disabled = false, id }) => (
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <select
          id={id}
          className="border p-2 pl-8 rounded w-full disabled:bg-gray-100"
          value={value}
          onChange={onChange}
          disabled={disabled}
        >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search tag/area/plant"
              value={q}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr
                    key={r.id}
                    className={`border-t ${selected?.id === r.id ? 'bg-blue-50' : ''}`}
                  ...
                  >
                    ...
                    <td className="p-2 flex gap-1">
                      ...
                      <Button
                        variant="ghost"
                        onClick={(e) => { e.stopPropagation(); deleteTag(r.id, r.tag_code); }}
                      >
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <IconSelect
              ...
              label="Area"
              value={form.area_uid}
              onChange={(e) => setForm({ ...form, area_uid: e.target.value })}
