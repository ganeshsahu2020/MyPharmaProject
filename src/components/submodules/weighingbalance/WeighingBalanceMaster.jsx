// ✅ File: src/components/submodules/weighingbalance/WeighingBalanceMaster.jsx
...
const WeighingBalanceMaster = () => {
  ...
  const loadDropdowns = async () => {
    try {
      const [p, sp, d, a] = await Promise.all([
        ...
        supabase
          .from('area_master')
          .select('id, area_id, area_name, department_uid'),
      ]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      least_count_percent: form.least_count_percent
        ? parseFloat(form.least_count_percent)
        : null,
      least_count_digits: form.least_count_digits
        ? parseInt(form.least_count_digits)
        : null,
      min_operating_capacity: form.min_operating_capacity
        ? parseFloat(form.min_operating_capacity)
        : null,
      max_operating_capacity: form.max_operating_capacity
        ? parseFloat(form.max_operating_capacity)
        : null,
  ...
  const renderInput = (key) => {
    ...
    if (key === 'subplant_uid')
      return (
        ...
        >
          ...
          {subplants
            .filter((sp) => sp.plant_uid === form.plant_uid)
            .map((sp) => (
    ...
    if (key === 'department_uid')
      return (
        ...
        >
          ...
          {departments
            .filter((dep) => dep.subplant_uid === form.subplant_uid)
            .map((dep) => (
    ...
    if (key === 'area_uid')
      return (
        <select {...commonProps}>
          ...
          {areas
            .filter((area) => area.department_uid === form.department_uid)
            .map((area) => (
    ...
    if (key === 'stamping_done_on' || key === 'stamping_due_on')
        ...
        placeholder={(() => {
          switch (key) {
            case 'balance_id':
              return 'e.g. BAL-001';
            case 'description':
              return '1 Kg Weighing Balance';
            case 'capacity':
              return 'e.g. 1';
            case 'ip_address':
              return 'Enter ip_address';
            case 'port_no':
              return 'Enter port_no';
            case 'make':
              return 'Mettler';
            case 'model':
              return 'XPR';
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSearch(
                  value ? balances.find((b) => b.id === value)?.balance_id || '' : '',
                );
              }}
            >
            ...
            filteredBalances.map((b) => (
              <div key={b.id} className="border rounded mb-4 p-3 shadow-sm bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  ...
                  <Row
                    label="Capacity"
                    value={formatWithLeastCountDigits(b.capacity, b.least_count_digits)}
                  />
                  ...
                  <Row
                    label="Min Capacity"
                    value={formatWithLeastCountDigits(
                      b.min_operating_capacity,
                      b.least_count_digits,
                    )}
                  />
                  <Row
                    label="Max Capacity"
                    value={formatWithLeastCountDigits(
                      b.max_operating_capacity,
                      b.least_count_digits,
                    )}
                  />
