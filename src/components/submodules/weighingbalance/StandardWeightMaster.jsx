// ✅ File: src/components/masters/StandardWeightMaster.jsx
...
const StandardWeightMaster = () => {
  ...
  useEffect(() => {
    const fetchUoms = async () => {
      const { data, error } = await supabase
        .from('vw_uom_master')
        .select('id,uom_code,uom,uom_name:uom') // ← alias uom as uom_name
  ...
  useEffect(() => {
    ...
    const fetchArea = async () => {
      const { data, error } = await supabase
        .from('area_master')
        .select('id,area_name')
        .eq('department_uid', selectedDepartment)
        .order('area_name');
  ...
  useEffect(() => {
    const fetchWb = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('id,weightbox_id,weightbox_type,area_uid')
        .eq('area_uid', selectedArea)
        .order('weightbox_id');
      if (error) {
        toast.error('Failed to load weight boxes');
        return;
      }
      setWeightboxes(data || []);
  ...
  const loadWeights = async () => {
    setLoading(true);
    const { data, error } = await supabase
      .from('vw_standard_weight_master')
      .select('*')
      .order('standard_weight_id');
    if (error) {
      toast.error('Failed to load standard weights');
      setLoading(false);
      return;
    }
    setWeights(data || []);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
            <label className="text-sm font-semibold flex flex-col">
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, standard_weight_id: e.target.value })}
                className="border p-1 rounded text-sm"
              />
            </label>
            ...
            <label className="text-sm font-semibold flex flex-col">
              ...
              <select
                value={form.standard_weight_type}
                onChange={(e) =>
                  setForm({ ...form, standard_weight_type: e.target.value })
                }
                className="border p-1 rounded text-sm"
              >
                {MATERIALS.map((m) => (
            ...
            <label className="text-sm font-semibold flex flex-col">
              ...
              >
                ...
                {uoms.map((u) => (
                  <option key={u.id} value={u.uom}>
                    {u.uom_code} - {u.uom_name} ({u.uom})
                  </option>
                ))}
              </select>
            </label>
            ...
            <label className="text-sm font-semibold flex flex-col">
              ...
              <select
                ...
                onChange={(e) => setSelectedWeightbox(e.target.value)}
                className="border p-1 rounded text-sm"
              >
                <option value="">Select WeightBox</option>
                {weightboxes.map((w) => (
                  <option key={w.id} value={w.id}>
                    {w.weightbox_id} — {String(w.weightbox_type || '')}
                  </option>
                ))}
              </select>
            </label>
            ...
            <label className="text-sm font-semibold flex flex-col">
              ...
              <input
                type="date"
                value={form.stamping_done_on}
                onChange={(e) =>
                  setForm({ ...form, stamping_done_on: e.target.value })
                }
                className="border p-1 rounded text-sm"
              />
            </label>
                    ...
                    <tr key={sw.id}>
                      ...
                      <td className="border p-1 flex gap-1">
                        <button
                          ...
                          className="bg-yellow-500 text-white p-1 rounded inline-flex"
                        >
                          <Edit3 size={14} />
                        </button>
                        <button
                          onClick={() => handleDelete(sw.id)}
                          className="bg-red-500 text-white p-1 rounded inline-flex"
                        >
                          <Trash2 size={14} />
                        </button>
                      </td>
