// src/components/submodules/weighingbalance/WeightBoxMaster.jsx
...
const WeightBoxMaster = () => {
  ...
  const onPlantChange = async (e) => {
    ...
    if (error) {
      toast.error("Failed to load subplants");
    }
    setSubplants(data || []);
  };
  ...
  const onEdit = async (wb) => {
    ...
    setForm({
      ...
      stamping_done_on: wb.stamping_done_on
        ? wb.stamping_done_on.slice(0, 10)
        : "",
    ...
    // Preload cascades for the selected chain
    if (wb.plant_uid) {
  ...
  const onSave = async () => {
    ...
    const payload = {
      weightbox_id: form.weightbox_id.trim(),
  ...
  const onDelete = async (id) => {
    if (!window.confirm("Delete this weight box?")) return;
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
            ...
            <label className="flex flex-col text-sm font-semibold">
              ...
              <input
                value={form.weightbox_id}
                onChange={(e) =>
                  setForm({ ...form, weightbox_id: e.target.value })
                }
            ...
            <label className="flex flex-col text-sm font-semibold">
              ...
              <select
                value={form.weightbox_type}
                onChange={(e) =>
                  setForm({ ...form, weightbox_type: e.target.value })
                }
                className="border p-1 rounded"
              >
                <option value="">Select Type</option>
                <option value="SS Weight Box">SS Weight Box</option>
                <option value="Analytical Weight Box">
            ...
            <label className="flex flex-col text-sm font-semibold">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, stamping_due_on: e.target.value })
                }
                className="border p-1 rounded"
              />
            </label>
            ...
            <label className="flex flex-col text-sm font-semibold">
              ...
              <select
                value={form.area_uid}
                onChange={(e) =>
                  setForm({ ...form, area_uid: e.target.value })
                }
                className="border p-1 rounded"
              >
        ...
        <div className="overflow-x-auto">
          <div className="flex items-center gap-2 mb-3">
            <input
              ...
              className="border p-1 rounded text-sm w-full sm:w-1/3"
            />
            {loading && (
              <span className="flex items-center gap-2 text-gray-500">
                <Loader2 className="animate-spin" />
                Loading weight boxes...
              </span>
            )}
          </div>
