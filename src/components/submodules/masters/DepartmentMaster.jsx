...
export default function DepartmentMaster(){
  ...
  async function fetchAll(){
    ...
    await toast.promise(p,{loading:'Loading departments...',success:'Loaded',error:(e)=>`Load failed: ${e.message}`});
  }
  ...
  useEffect(()=>{
    if(form.subplant_uid && !form.plant_uid){
      ...
      if(sp){setForm((f)=>({...f,plant_uid:sp.plant_uid}));}
    }
  ...
  async function handleSave(e){
    ...
    if(!sp){toast.error('Subplant not found');return;}
    if(form.plant_uid && sp.plant_uid!==form.plant_uid){toast.error('Subplant does not belong to selected plant');return;}

    setSaving(true);

    if(!editing){
      const op=supabase.from('department_master').select('id').eq('department_id',form.department_id).maybeSingle()
        .then(({data})=>{
          if(data){throw new Error('Department ID must be unique');}
        })
        .then(()=>supabase.from('department_master').insert([{
          id:uuidv4(),
        ...
        .then(({error})=>{
          ...
          setForm({id:null,department_id:'',department_name:'',plant_uid:'',subplant_uid:'',status:'Active'});
          setEditing(false);
          return fetchAll();
        });
      await toast.promise(op,{loading:'Saving department...',success:'Department added',error:(e)=>e.message});
    }else{
      const op=supabase.from('department_master').select('id').eq('department_id',form.department_id).neq('id',form.id).maybeSingle()
        .then(({data})=>{
          if(data){throw new Error('Department ID must be unique');}
        })
        .then(()=>supabase.from('department_master').update({
          department_id:form.department_id,
          department_name:form.department_name,
          subplant_uid:form.subplant_uid,
          status:form.status
      ...
      await toast.promise(op,{loading:'Updating department...',success:'Department updated',error:(e)=>e.message});
    }
  ...
  async function handleDelete(id){
    const op=supabase.from('department_master').delete().eq('id',id)
      .then(({error})=>{
        if(error) throw new Error(error.message);
        return fetchAll();
      });
  ...
  return (
    <div className="p-3 max-w-6xl mx-auto">
      <h2 className="text-lg font-bold mb-3 text-blue-700 flex items-center gap-2">
        <Layers className="h-5 w-5"/>Department Master
      </h2>
        ...
        <form onSubmit={handleSave} className="bg-white border p-3 rounded mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div>
            ...
            >
              ...
              {plants.map((pl)=>(
                <option key={pl.id} value={pl.id}>{pl.plant_id} - {pl.description}</option>
              ))}
            </IconSelect>
          </div>
            ...
            >
              ...
              {filteredSubplants.map((sp)=>(
                <option key={sp.id} value={sp.id}>
                  {sp.subplant_id} - {sp.subplant_name}{sp.description?` (${sp.description})`:''}
                </option>
              ))}
            </IconSelect>
          </div>
          ...
          <div className="flex items-end gap-2">
            ...
            {editing&&(
              <button type="button" className="inline-flex items-center gap-2 bg-gray-400 text-white px-3 py-2 rounded text-sm" onClick={()=>{setForm({id:null,department_id:'',department_name:'',plant_uid:'',subplant_uid:'',status:'Active'});setEditing(false);}}>
                Cancel
              </button>
            )}
          </div>
                ...
                {loading?(
                  ...
                  filteredRows.map((r)=>(
                    <tr key={r.department_uid}>
                      ...
                      <td className="p-2 border"><Badge status={r.department_status}/></td>
                      <td className="p-2 border">
                        <div className="inline-flex gap-2">
                          ...
                          <button onClick={()=>handleDelete(r.department_uid)} className="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs hover:bg-rose-50 hover:border-rose-300">
                  ...
                  <tr>
                    <td colSpan={6} className="p-4 text-center text-gray-500">No Departments Found</td>
                  </tr>
                )}
              </tbody>
            </table>
