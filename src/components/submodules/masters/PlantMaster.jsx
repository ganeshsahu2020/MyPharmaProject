...
import {
  Hash, ClipboardList, CheckCircle2, MapPin,
...
const PlantMaster=()=>{
  ...
  const resetFormDom=()=>{
    [plantIdRef,descriptionRef,statusRef,taxRef,licenseRef,gs1Ref,addressRef].forEach(r=>{if(r.current) r.current.value='';});
    if(statusRef.current) statusRef.current.value='Active';
    setCurrent({id:null,plant_id:'',description:'',status:'Active',tax_reg_no:'',license_no:'',gs1_prefix:'',address1:''});
    setEditingId(null);
    setFormKey(k=>k+1);
  };
  ...
  const handleEdit=(plant)=>{
    ...
    if(descriptionRef.current) descriptionRef.current.value=plant.plant_description||'';
  ...
  return (
    <div className="p-3 max-w-7xl mx-auto">
      ...
      {activeTab==='manage'&&(
        <form key={formKey} onSubmit={handleSubmit} className="bg-gray-50 p-3 rounded mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          <InputIconWrapper icon={Hash} color="text-blue-600" label="Plant ID">
            <input type="text" ref={plantIdRef} defaultValue={current.plant_id} className="border pl-7 p-2 rounded text-sm w-full" required/>
          </InputIconWrapper>

          <InputIconWrapper icon={ClipboardList} color="text-green-600" label="Description">
            <input type="text" ref={descriptionRef} defaultValue={current.plant_description} className="border pl-7 p-2 rounded text-sm w-full" required/>
          </InputIconWrapper>

          <InputIconWrapper icon={CheckCircle2} color="text-purple-600" label="Status">
            <select ref={statusRef} defaultValue={current.plant_status||'Active'} className="border pl-7 p-2 rounded text-sm w-full">
          ...
          <InputIconWrapper icon={FileText} color="text-pink-600" label="Tax Registration No">
            <input type="text" ref={taxRef} defaultValue={current.tax_reg_no} className="border pl-7 p-2 rounded text-sm w-full"/>
          </InputIconWrapper>

          <InputIconWrapper icon={FileText} color="text-yellow-600" label="License No">
            <input type="text" ref={licenseRef} defaultValue={current.license_no} className="border pl-7 p-2 rounded text-sm w-full"/>
          </InputIconWrapper>

          <InputIconWrapper icon={Hash} color="text-indigo-600" label="GS1 Prefix">
            <input type="text" ref={gs1Ref} defaultValue={current.gs1_prefix} className="border pl-7 p-2 rounded text-sm w-full"/>
          </InputIconWrapper>

          <InputIconWrapper icon={MapPin} color="text-red-600" label="Address">
            <input type="text" ref={addressRef} defaultValue={current.address1} className="border pl-7 p-2 rounded text-sm w-full"/>
          </InputIconWrapper>
          ...
          <div className="mt-2 col-span-full flex items-center gap-3">
            ...
            {editingId&&(
              <button type="button" onClick={resetFormDom} className="bg-gray-400 text-white px-3 py-2 rounded text-sm">
                Cancel
              </button>
            )}
          </div>
      ...
      {activeTab==='preview'&&(
        <>
          <div className="mb-3">
            <input type="text" placeholder="Search Plant..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="border p-2 rounded w-full text-sm"/>
          </div>
                ...
                {loading?(
                  [...Array(3)].map((_,i)=>(
                    <tr key={i} className="animate-pulse">
                      {Array(8).fill(0).map((_,j)=>(<td key={j} className="border p-2 bg-gray-200">&nbsp;</td>))}
                    </tr>
                  ))
                  ...
                  filteredPlants.map((plant)=>(
                    <tr key={plant.plant_uid}>
                      ...
                      <td className="border p-2">
                        <span className={`px-2 py-1 text-xs rounded ${plant.plant_status==='Active'?'bg-green-100 text-green-800':'bg-red-100 text-red-800'}`}>
                          {plant.plant_status}
                        </span>
                      </td>
                      ...
                      <td className="border p-2 flex gap-2 justify-center">
                        <button onClick={()=>handleEdit(plant)} className="p-1 bg-yellow-500 rounded hover:bg-yellow-600">
                          <Pencil className="text-white" size={16}/>
                        </button>
                        <button onClick={()=>handleDelete(plant.plant_uid)} className="p-1 bg-red-500 rounded hover:bg-red-600">
                  ...
                  <tr>
                    <td colSpan="8" className="text-center p-2 text-gray-500">No plants found</td>
                  </tr>
                )}
              </tbody>
            </table>
