...
import {
  Layers,Hash,Tags,Building2,Factory,ChevronDown,Search,
  CheckCircle2,XCircle,Loader2,Pencil,Trash2,ShieldCheck,ShieldAlert,FileText
} from 'lucide-react';

/* ---------- UI Helpers (prevent icon overlap) ---------- */
const IconInput=({icon:Icon,value,onChange,placeholder,type='text',required=false,color='text-purple-500'})=>(
...
const IconSelect=({icon:Icon,value,onChange,children,disabled=false,leftColor='text-blue-500'})=>(
  <div className="relative">
...
const TypePill=({type})=>{
  ...
  if(t==='classified area'){
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-indigo-100 text-indigo-700 border border-indigo-200"><ShieldCheck className="h-3.5 w-3.5"/>Classified Area</span>;
  }
  if(t==='non-classified area'){
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-700 border border-amber-200"><ShieldAlert className="h-3.5 w-3.5"/>Non-Classified Area</span>;
  }
  return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700 border">—</span>;
};
...
export default function AreaMaster(){
  ...
  async function fetchAll(){
    ...
    const p=Promise.all([
      ...
      supabase.from('subplant_master').select('id,subplant_id,subplant_name,plant_uid,description,status').order('subplant_id',{ascending:true}),
    ...
    await toast.promise(p,{loading:'Loading areas...',success:'Loaded',error:(e)=>`Load failed: ${e.message}`});
  }
  ...
  async function handleSave(e){
    ...
    if(!form.area_id||!form.area_name||!form.department_uid){
      toast.error('Area ID, Name, and Department are required');return;
    }

    setSaving(true);

    if(!editing){
      const op=supabase.from('area_master').select('id').eq('area_id',form.area_id).eq('department_uid',form.department_uid).maybeSingle()
        .then(({data})=>{ if(data){throw new Error('Area ID must be unique within the selected Department');} })
        .then(()=>supabase.from('area_master').insert([{
          id:uuidv4(),
        ...
        .then(({error})=>{
          ...
          setForm({id:null,area_id:'',area_name:'',description:'',area_type:'Classified Area',plant_uid:'',subplant_uid:'',department_uid:'',status:'Active'});
          setEditing(false);
          return fetchAll();
        });
      await toast.promise(op,{loading:'Saving area...',success:'Area added',error:(e)=>e.message});
    }else{
      const op=supabase.from('area_master').select('id').eq('area_id',form.area_id).eq('department_uid',form.department_uid).neq('id',form.id).maybeSingle()
        .then(({data})=>{ if(data){throw new Error('Area ID must be unique within the selected Department');} })
        .then(()=>supabase.from('area_master').update({
          area_id:form.area_id,
          area_name:form.area_name,
          description:form.description||null,
          area_type:form.area_type||null,
      ...
      await toast.promise(op,{loading:'Updating area...',success:'Area updated',error:(e)=>e.message});
    }
  ...
  async function handleDelete(id){
    const op=supabase.from('area_master').delete().eq('id',id)
      .then(({error})=>{ if(error) throw new Error(error.message); return fetchAll(); });
  ...
  return (
    <div className="p-3 max-w-6xl mx-auto">
      <h2 className="text-lg font-bold mb-3 text-blue-700 flex items-center gap-2">
        <Layers className="h-5 w-5"/>Area Master
      </h2>
      ...
      {activeTab==='manage'&&(
        <form onSubmit={handleSave} className="bg-white border p-3 rounded mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div>
            ...
            <IconInput icon={Hash} required value={form.area_id} onChange={(e)=>setForm({...form,area_id:e.target.value})} placeholder="AREA-001" color="text-purple-600"/>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1">Area Name</label>
            <IconInput icon={Tags} required value={form.area_name} onChange={(e)=>setForm({...form,area_name:e.target.value})} placeholder="Dispensing Room" color="text-orange-500"/>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1">Area Type</label>
            <IconSelect icon={ShieldCheck} value={form.area_type} onChange={(e)=>setForm({...form,area_type:e.target.value})} leftColor={form.area_type==='Classified Area'?'text-indigo-600':'text-amber-600'}>
          ...
          <div>
            ...
            <IconSelect icon={Building2} value={form.plant_uid||''} onChange={(e)=>setForm((f)=>({...f,plant_uid:e.target.value,subplant_uid:'',department_uid:''}))} leftColor="text-blue-600">
              <option value="">Select Plant</option>
              {plants.map((pl)=>(<option key={pl.id} value={pl.id}>{pl.plant_id} - {pl.description}</option>))}
            </IconSelect>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1">SubPlant</label>
            <IconSelect icon={Factory} value={form.subplant_uid||''} onChange={(e)=>setForm((f)=>({...f,subplant_uid:e.target.value,department_uid:''}))} disabled={!form.plant_uid} leftColor="text-green-600">
              <option value="">{form.plant_uid?'Select SubPlant':'Select a Plant first'}</option>
              {subplantsByPlant.map((sp)=>(<option key={sp.id} value={sp.id}>{sp.subplant_id} - {sp.subplant_name}{sp.description?` (${sp.description})`:''}</option>))}
            </IconSelect>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1">Department</label>
            <IconSelect icon={Layers} value={form.department_uid||''} onChange={(e)=>setForm({...form,department_uid:e.target.value})} disabled={!form.subplant_uid} leftColor="text-fuchsia-600">
              <option value="">{form.subplant_uid?'Select Department':'Select a SubPlant first'}</option>
              {departmentsBySubplant.map((d)=>(<option key={d.id} value={d.id}>{d.department_id} - {d.department_name}</option>))}
            </IconSelect>
          </div>

          <div className="sm:col-span-2 lg:col-span-3">
            <label className="block text-xs font-medium mb-1">Description</label>
            <IconTextarea icon={FileText} value={form.description} onChange={(e)=>setForm({...form,description:e.target.value})} placeholder="Short description of the area..."/>
          </div>
          ...
          <div className="flex items-end gap-2">
            ...
            {editing&&(
              <button type="button" className="inline-flex items-center gap-2 bg-gray-400 text-white px-3 py-2 rounded text-sm" onClick={()=>{setForm({id:null,area_id:'',area_name:'',description:'',area_type:'Classified Area',plant_uid:'',subplant_uid:'',department_uid:'',status:'Active'});setEditing(false);}}>
                Cancel
              </button>
            )}
          </div>
                ...
                {loading?(
                  ...
                  filteredRows.map((r)=>(
                    <tr key={r.area_uid}>
                      ...
                      <td className="p-2 border"><TypePill type={r.area_type}/></td>
                      ...
                      <td className="p-2 border"><Badge status={r.area_status}/></td>
                      <td className="p-2 border">
                        <div className="inline-flex gap-2">
                          <button onClick={()=>handleEdit(r)} className="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs hover:bg-yellow-50 hover:border-yellow-300">
                            <Pencil className="h-3.5 w-3.5 text-yellow-500"/>Edit
                          </button>
                          <button onClick={()=>handleDelete(r.area_uid)} className="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs hover:bg-rose-50 hover:border-rose-300">
                  ...
                  <tr>
                    <td colSpan={8} className="p-4 text-center text-gray-500">No Areas Found</td>
                  </tr>
                )}
              </tbody>
            </table>
