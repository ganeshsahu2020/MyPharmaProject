...
export default function UomMaster() {
  ...
  async function loadAll() {
    ...
    const op = Promise.all([
      ...
      // explicit columns from TABLE
      supabase
        .from("uom_master")
        .select(
          "id, plant_uid, uom_code, uom_name, uom, numerator_value, denominator_value, status"
        )
  ...
  const filteredRows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter((r) => {
        const name = (r.uom_name ?? "").toLowerCase();
        const code = (r.uom_code ?? "").toLowerCase();
        const sym  = (r.uom ?? "").toLowerCase();
        return name.includes(t) || code.includes(t) || sym.includes(t);
      });
    }
    return data;
  ...
  async function handleSave(e) {
    ...
    if (!form.uom_code || !form.uom_name) {
      toast.error("UOM Code & Name are required");
      return;
    }
      ...
      {activeTab === "manage" && (
        ...
        >
          ...
          <div>
            ...
            <IconNumber
              icon={Scale}
              value={form.numerator_value}
              onChange={(e) => setForm({ ...form, numerator_value: e.target.value })}
              placeholder="1"
          ...
          <div>
            ...
            <IconNumber
              icon={Scale}
              value={form.denominator_value}
              onChange={(e) => setForm({ ...form, denominator_value: e.target.value })}
              placeholder="1"
              color="text-rose-600"
            />
          </div>
                  ...
                  filteredRows.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td className="p-2 border">
                        {plants.find((p) => p.id === r.plant_uid)?.plant_id || r.plant_uid}
                      </td>
