...
import {
  Hash, ClipboardList, Building2, CheckCircle2, XCircle,
  ChevronDown, Search, Loader2, Pencil, Trash2
} from 'lucide-react';

/* ---------- UI Helpers (prevent icon overlap) ---------- */
const IconInput=({icon:Icon,inputRef,defaultValue,placeholder,type='text',required=false,color='text-indigo-600'})=>(
...
const IconSelect=({icon:Icon,selectRef,defaultValue,children,disabled=false,leftColor='text-blue-600',required=false})=>(
  <div className="relative">
...
const SubPlantMaster=()=>{
  ...
  async function loadAll(){
    ...
    await toast.promise(op,{loading:'Loading subplants...',success:'Loaded',error:(e)=>`Load failed: ${e.message}`});
  }
  ...
  const resetFormDom=()=>{
    ...
    setCurrent({id:null,subplant_id:'',subplant_name:'',description:'',plant_uid:'',status:'Active'});
    setEditingId(null);
    setFormKey(k=>k+1);
  };
  ...
  async function handleSave(e){
    ...
    if(!payload.subplant_id||!payload.subplant_name||!payload.plant_uid){
      toast.error('SubPlant ID, Name & Plant are required'); return;
    }
    ...
    try{
      if(!editingId){
        // uniqueness UX
        const {data:exists} = await supabase.from('subplant_master').select('id').eq('subplant_id',payload.subplant_id).maybeSingle();
        if(exists){ throw new Error('SubPlant ID must be unique'); }
        const {error} = await supabase.from('subplant_master').insert([{id:uuidv4(),...payload}]);
        if(error) throw error;
        await toast.promise(Promise.resolve(),{loading:'',success:'SubPlant added',error:''});
      }else{
        const {data:exists} = await supabase.from('subplant_master').select('id').eq('subplant_id',payload.subplant_id).neq('id',editingId).maybeSingle();
        if(exists){ throw new Error('SubPlant ID must be unique'); }
        const {error} = await supabase.from('subplant_master').update(payload).eq('id',editingId);
        if(error) throw error;
        await toast.promise(Promise.resolve(),{loading:'',success:'SubPlant updated',error:''});
      }
      resetFormDom();
  ...
  async function handleDelete(id){
    if(!id){toast.error('Missing ID for delete'); return;}
    if(!window.confirm('Delete this SubPlant?')) return;
    const op=supabase.from('subplant_master').delete().eq('id',id)
  ...
  return (
    <div className="p-3 max-w-7xl mx-auto">
      ...
      <div className="flex gap-3 mb-3">
        <button onClick={()=>setActiveTab('manage')} className={`px-3 py-1 rounded ${activeTab==='manage'?'bg-blue-600 text-white':'bg-gray-200'}`} type="button">Manage</button>
        <button onClick={()=>setActiveTab('preview')} className={`px-3 py-1 rounded ${activeTab==='preview'?'bg-blue-600 text-white':'bg-gray-200'}`} type="button">Preview All</button>
      </div>

      {activeTab==='manage'&&(
        <form onSubmit={handleSave} key={formKey} className="bg-white border p-3 rounded mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div>
            <label className="block text-xs font-medium mb-1">SubPlant ID</label>
            <IconInput icon={Hash} inputRef={subplantIdRef} defaultValue={current.subplant_id} placeholder="SUBP-001" color="text-indigo-600" required/>
          </div>

          <div>
            <label className="block text-xs font-medium mb-1">SubPlant Name</label>
            <IconInput icon={ClipboardList} inputRef={subplantNameRef} defaultValue={current.subplant_name} placeholder="Granulation Block" color="text-green-600" required/>
          </div>

          <div className="sm:col-span-2 lg:col-span-3">
            <label className="block text-xs font-medium mb-1">Description</label>
            <IconInput icon={ClipboardList} inputRef={descriptionRef} defaultValue={current.description} placeholder="Optional description..." color="text-purple-600"/>
          </div>
          ...
          <div>
            ...
            <IconSelect icon={current.status==='Active'?CheckCircle2:XCircle} selectRef={statusRef} defaultValue={current.status||'Active'} leftColor={current.status==='Active'?'text-emerald-600':'text-rose-600'}>
          ...
          <div className="flex items-end gap-2">
            ...
            {editingId&&(
              <button type="button" className="inline-flex items-center gap-2 bg-gray-400 text-white px-3 py-2 rounded text-sm" onClick={()=>{resetFormDom();}}>
                Cancel
              </button>
            )}
          </div>
                ...
                {loading?(
                  Array.from({length:6}).map((_,i)=>(
                    <tr key={`sk-${i}`} className="animate-pulse">
                      <td className="border p-2"><div className="h-4 bg-gray-200 rounded w-24"/></td>
                      <td className="border p-2"><div className="h-4 bg-gray-200 rounded w-40"/></td>
                      <td className="border p-2"><div className="h-4 bg-gray-200 rounded w-48"/></td>
                      <td className="border p-2"><div className="h-4 bg-gray-200 rounded w-44"/></td>
                      <td className="border p-2"><div className="h-4 bg-gray-200 rounded w-16"/></td>
                      <td className="border p-2"><div className="h-7 bg-gray-200 rounded w-28"/></td>
                    </tr>
                  ))
                  ...
                  filteredRows.map((r)=>(
                    <tr key={r.subplant_uid}>
                      ...
                      <td className="border p-2">{r.plant_id} - {r.plant_description}</td>
                      <td className="border p-2"><Badge status={r.subplant_status}/></td>
                      <td className="border p-2">
                        <div className="inline-flex gap-2">
                          ...
                          <button onClick={()=>handleDelete(r.subplant_uid)} className="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs hover:bg-rose-50 hover:border-rose-300">
                  ...
                  <tr>
                    <td colSpan={6} className="p-4 text-center text-gray-500">No SubPlants Found</td>
                  </tr>
                )}
              </tbody>
            </table>
