...
export default function LocationMaster() {
  ...
  const filteredSubplants = useMemo(
    () => (selectedPlant ? subplants.filter((s) => s.plant_uid === selectedPlant) : []),
    [selectedPlant, subplants]
  );

  const filteredDepartments = useMemo(
    () => (selectedSubplant ? departments.filter((d) => d.subplant_uid === selectedSubplant) : []),
    [selectedSubplant, departments]
  );

  const filteredAreas = useMemo(
    () => (selectedDepartment ? areas.filter((a) => a.department_uid === selectedDepartment) : []),
  ...
  const filteredRows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter(
        (r) => r.location_id?.toLowerCase().includes(t) || r.location_name?.toLowerCase().includes(t)
      );
    }
    return data;
  ...
  async function handleSave(e) {
    ...
    if (!form.location_id || !form.location_name || !selectedArea) {
      toast.error('Location ID, Name and Area are required');
      return;
    }
    ...
    try {
      if (!form.id) {
        ...
        await toast.promise(op, {
          ...
          error: (e) => e.error?.message || 'Save failed',
        });
      } else {
        ...
        await toast.promise(op, {
          ...
          error: (e) => e.error?.message || 'Update failed',
        });
      }
      resetForm();
                ...
                {loading ? (
                  ...
                  filteredRows.map((r) => (
                    <tr key={r.location_uid}>
                      ...
                      <td className="p-2 border">
                        <TypeBadge type={r.location_type} />
                      </td>
                      <td className="p-2 border">
                      ...
                      <td className="p-2 border">
                        <StatusBadge status={r.location_status} />
                      </td>
                      <td className="p-2 border">
                        <div className="inline-flex gap-2">
                          ...
                          <button
                            onClick={() => handleDelete(r.location_uid)}
