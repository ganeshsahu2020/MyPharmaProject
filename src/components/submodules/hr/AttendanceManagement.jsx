...
const parseIntSafe=(v,fallback)=>{ const n=parseInt(v,10); return Number.isFinite(n)?n:fallback; };

// minutes → hours helpers
const minToHours=(m)=>((Number(m)||0)/60).toFixed(2);
const overtimeHoursFromMin=(m,baseMin)=>((Math.max(0,(Number(m)||0)-(Number(baseMin)||0)))/60).toFixed(2);
...
const AttendanceManagement=()=>{
  ...
  const nameById=useMemo(()=>{
    ...
    for(const p of people){
      m.set(p.id,`${p.employee_id?`[${p.employee_id}] `:''}${p.first_name||''} ${p.last_name||''}`.trim());
    }
    return m;
  },[people]);

  // attendance browser
  const [range,setRange]=useState({start:isoDate(new Date(Date.now()-7*86400000)),end:isoDate()});
  ...
  // "create on behalf"
  const [newCorr,setNewCorr]=useState({employee_uid:'',date:isoDate(),in:'',out:'',reason:''});
      ...
      if((roles||[]).length===0 && authEmail){
        try{
          const {data}=await supabase
            ...
            .eq('email',authEmail)
            .maybeSingle();
          if(data){ roles=normalizeRoles(data.role); who={email:authEmail,roles}; }
        }catch(_){}
        if((roles||[]).length===0){
          try{
            const {data}=await supabase
  ...
  useEffect(()=>{ loadLeaves(); /* eslint-disable-next-line */ },[statusFilter,leaveSearch]);
  ...
  async function loadDirectory(){
    try{
      ...
      if(data && data.length){ setPeople(data); return; }
    }catch(_){}
    try{
      const {data}=await supabase
        .from('vw_user_management')
  ...
  async function loadLeaves(){
    ...
    try{
      let q=supabase
        ...
        .select('id, employee_uid, date_from, date_to, leave_type, status, reason, hr_comment, hr_private_note, created_at')
        .order('created_at',{ascending:false});
      ...
      if(leaveSearch.trim()){
        ...
        rows=rows.filter((r)=>{
          ...
          return nm.includes(s)||(r.leave_type||'').toLowerCase().includes(s)||(r.reason||'').toLowerCase().includes(s);
        });
      }
      setLeaveRows(rows);
  ...
  function exportCorrectionsCsv(){
    ...
    downloadCsv(`corrections_pending_${range.start}_${range.end}.csv`,rows,
  ...
  async function createCorrectionForEmployee(){
    if(!canHR) return alert('Not authorized');
    if(!newCorr.employee_uid||!newCorr.reason.trim()) return alert('Employee & reason required');
    ...
    try{
      const {data,error}=await supabase.from('attendance_correction_request').insert([payload]).select().single();
  ...
  async function decideCorrection(id,decision){
    ...
    const {data,error}=await supabase.rpc('hr_correction_decide',{p_id:id,p_decision:decision,p_hr_comment:comment});
    console.log('hr_correction_decide',{status:error?.status,code:error?.code,details:error?.details,hint:error?.hint,message:error?.message,data});
  ...
  async function saveLeaveRow(r){
    ...
    setFlash(null);
    try{
      const {error}=await supabase.rpc('hr_leave_set_status',{
        p_leave_id:r.id,
        p_status:r.status,
      ...
      if(error){
        console.error('hr_leave_set_status',error);
        setFlash({kind:'error',text:`Save failed: ${error.message}`});
      }else{
      ...
      console.error(e);
      setFlash({kind:'error',text:`Network error: ${e.message}`});
    }finally{
      setBusy(false);
    }
  }
  ...
  const filteredAttendance=useMemo(()=>{
    ...
    return attendance.filter((r)=>{
      const nm=(nameById.get(r.employee_uid)||'').toLowerCase();
      const bag=`${nm} ${r.status??''} ${r.date} ${toTime(r.check_in)} ${toTime(r.check_out)} ${minToHours(r.total_work_minutes)} ${minToHours(r.total_break_minutes)}`.toLowerCase();
      return bag.includes(t);
    });
  },[attendance,search,nameById]);

  // ---------- HR dashboard summary (configurable OT) ----------
  const attSummary=useMemo(()=>{
    ...
    const overtimeMin=rows.reduce((a,r)=>a+Math.max(0,(Number(r.total_work_minutes)||0)-ovBaseMin),0);
  ...
  const saveOtBase=async()=>{
    ...
    if(!Number.isFinite(hours) || hours<0 || hours>24) {
      alert('Enter a valid number of hours between 0 and 24.');
      return;
    }
    ...
    const {error}=await supabase.rpc('app_set_overtime_base_min',{p_value:newMin});
    if(error){
      setOvBaseMin(old);
      setSavingCfg(false);
      alert(error.message);
    ...
    <div className="p-3 space-y-4">
        ...
        <div className="mt-4 border rounded p-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[200px]">
              <label className="block text-xs mb-1">Search (name/status/date/time)</label>
              <Input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="e.g. [EMP1001] Alex present 2025-08-10"/>
            </div>
            <div className="flex items-end gap-2">
              <div className="flex items-center gap-2 mr-2">
                <div className="text-[11px] text-gray-600">OT base: {(ovBaseMin/60).toFixed(2)}h</div>
                {canHR && (
                  <button
                    type="button"
                    onClick={()=>{setCfgVal((ovBaseMin/60).toFixed(2)); setShowCfg(true);}}
                    className="ml-2 underline text-blue-600"
                    title="Edit overtime base"
                  >
                    edit
                  </button>
                )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-2 mt-3">
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Days</div><div className="font-semibold">{attSummary.days}</div></div>
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Present Days</div><div className="font-semibold">{attSummary.presentDays}</div></div>
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Work (h)</div><div className="font-semibold">{attSummary.workH}</div></div>
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Break (h)</div><div className="font-semibold">{attSummary.breakH}</div></div>
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Overtime (h)</div><div className="font-semibold">{attSummary.overtimeH}</div></div>
            <div className="border rounded p-2 text-center"><div className="text-xs text-gray-500">Avg/Day (h)</div><div className="font-semibold">{attSummary.avgH}</div></div>
          </div>
            ...
            <table className="w-full text-sm border">
              ...
              <tbody>
                {loadingAtt?(
                  ...
                  <tr><td colSpan={9} className="p-3 text-center text-gray-500">No records</td></tr>
                ):(
                  filteredAttendance.map((r)=>(
                    <tr key={`${r.employee_uid}-${r.date}`}>
                      ...
                      <td className="p-2 border">{overtimeHoursFromMin(r.total_work_minutes,ovBaseMin)}</td>
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card className="p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="font-semibold text-sm">Pending Missed Punch Requests</div>
              <Button variant="outline" onClick={loadCorrections}>Refresh</Button>
            </div>

            {/* HR create on behalf */}
            <div className="mb-3 border rounded p-2">
              <div className="font-semibold text-sm mb-1">Create Correction (on behalf)</div>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                <select className="border rounded p-2 text-sm" value={newCorr.employee_uid} onChange={(e)=>setNewCorr((s)=>({...s,employee_uid:e.target.value}))}>
                  <option value="">Select employee…</option>
                  {people.map((p)=>(
                    <option key={p.id} value={p.id}>{p.employee_id?`[${p.employee_id}] `:''}{p.first_name} {p.last_name}</option>
                  ))}
                </select>
                <input type="date" className="border rounded p-2 text-sm" value={newCorr.date} onChange={(e)=>setNewCorr((s)=>({...s,date:e.target.value}))}/>
                <input type="time" className="border rounded p-2 text-sm" value={newCorr.in} onChange={(e)=>setNewCorr((s)=>({...s,in:e.target.value}))}/>
                <input type="time" className="border rounded p-2 text-sm" value={newCorr.out} onChange={(e)=>setNewCorr((s)=>({...s,out:e.target.value}))}/>
                <input type="text" className="border rounded p-2 text-sm" placeholder="Reason" value={newCorr.reason} onChange={(e)=>setNewCorr((s)=>({...s,reason:e.target.value}))}/>
              </div>
              <div className="mt-2">
                <Button disabled={!canHR||creatingCorr} onClick={createCorrectionForEmployee}>{creatingCorr?'Creating…':'Create'}</Button>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border">
                ...
                <tbody>
                  {loadingCorr?(
                    <>
                      <SkelRow cols={6}/><SkelRow cols={6}/><SkelRow cols={6}/>
                    </>
                  ):corrPending.length===0?(
                    <tr><td colSpan={6} className="p-3 text-center text-gray-500">None</td></tr>
                  ):corrPending.map((r)=>(
                    <tr key={r.id}>
                      <td className="p-2 border">{nameById.get(r.employee_uid)||r.employee_uid}</td>
                      ...
                      <td className="p-2 border">
                        <textarea className="border rounded p-1 text-sm w-full" rows={2} placeholder="Optional" value={corrComment[r.id]||''} onChange={(e)=>setCorrComment((s)=>({...s,[r.id]:e.target.value}))}/>
                      </td>
                      <td className="p-2 border">
                        <div className="flex gap-2">
                          <Button disabled={!canHR||!!deciding[r.id]} onClick={()=>decideCorrection(r.id,'approved')}>{deciding[r.id]?'Working…':'Approve'}</Button>
                          <Button disabled={!canHR||!!deciding[r.id]} variant="destructive" onClick={()=>decideCorrection(r.id,'rejected')}>{deciding[r.id]?'Working…':'Reject'}</Button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <Card className="p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="font-semibold text-sm">Leave Requests</div>
              <div className="flex items-end gap-2">
                <div>
                ...
                <div>
                  ...
                  <Input value={leaveSearch} onChange={(e)=>setLeaveSearch(e.target.value)} placeholder="Name, EmpID, Type…" className="w-56"/>
                </div>
                <Button variant="outline" onClick={loadLeaves}>Refresh</Button>
              </div>
            </div>

            {flash&&(
              <div className={`mt-2 text-sm rounded border p-2 ${flash.kind==='success'?'bg-green-50 text-green-700 border-green-200':'bg-red-50 text-red-700 border-red-200'}`}>{flash.text}</div>
            )}
            ...
            <div className="overflow-x-auto mt-3">
              <table className="w-full text-sm border">
                ...
                <tbody>
                  {loadingLeave?(
                    <>
                      <SkelRow cols={7}/><SkelRow cols={7}/><SkelRow cols={7}/>
                    </>
                  ):leaveRows.length===0?(
                    <tr><td colSpan={7} className="p-3 text-center text-gray-500">No rows</td></tr>
                  ):leaveRows.map((r)=>(
                    <tr key={r.id} className="align-top">
                      <td className="p-2 border"><div className="font-medium">{nameById.get(r.employee_uid)||r.employee_uid}</div></td>
                      <td className="p-2 border">{r.date_from} → {r.date_to}</td>
                      <td className="p-2 border">{r.leave_type}</td>
                      <td className="p-2 border">
                        <select className="border rounded p-1 text-sm" value={r.status} onChange={(e)=>setLeaveRows((prev)=>prev.map((x)=>(x.id===r.id?{...x,status:e.target.value}:x)))}>
                          {STATUSES.map((s)=>(<option key={s} value={s}>{s}</option>))}
                        </select>
                      </td>
                      <td className="p-2 border">
                        <textarea className="border rounded p-2 text-sm w-72" rows={2} value={r.hr_comment||''} onChange={(e)=>setLeaveRows((prev)=>prev.map((x)=>(x.id===r.id?{...x,hr_comment:e.target.value}:x)))} placeholder="Visible to employee"/>
                      </td>
                      <td className="p-2 border">
                        <textarea className="border rounded p-2 text-sm w-72" rows={2} value={r.hr_private_note||''} onChange={(e)=>setLeaveRows((prev)=>prev.map((x)=>(x.id===r.id?{...x,hr_private_note:e.target.value}:x)))} placeholder="Internal (HR only)"/>
                      </td>
                      <td className="p-2 border">
                        <Button disabled={busy||!canHR} onClick={()=>saveLeaveRow(r)}>{busy?'Saving…':'Save'}</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <Modal open={showCfg} onClose={()=>setShowCfg(false)}>
        <div className="p-4 space-y-3">
          <div className="text-lg font-semibold">Overtime Base</div>
          <div className="text-sm text-gray-600">Set the base hours per day before overtime starts.</div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" onClick={()=>setShowCfg(false)} disabled={savingCfg}>Cancel</Button>
            <Button onClick={saveOtBase} disabled={savingCfg}>{savingCfg?'Saving…':'Save'}</Button>
          </div>
        </div>
      </Modal>
    </div>
