...
import { CheckCircle2, Clock, ClipboardCopy, ShieldCheck, Printer, QrCode, ArrowLeft } from 'lucide-react';
...
const pickWO = (raw) => {
  ...
  return m ? m[1] : null; // return HR body (e.g., "1SJ44WH")
};
...
export default function PMWorkOrderDetail() {
  ...
  // 1) Resolve ref → UUID token (and compute/display HR)
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        ...
        if (isUuid(raw)) {
          ...
          if (body) {
            hr = hrWrap(body);
            // Try RPC with p_code first; fallback to p_hr (both variants supported)
            let got = null;
            try {
              const r1 = await supabase.rpc('pm_resolve_wo_hr', { p_code: body });
              if (!r1.error && r1.data) got = r1.data;
            } catch {}
            if (!got) {
              try {
                const r2 = await supabase.rpc('pm_resolve_wo_hr', { p_hr: body });
        ...
        setWoToken(token);
        if (!hr) {
          // derive display HR from token for UI: same scheme you used in PMScheduler
        ...
        let payload = null;
        try {
          const { data: rows, error } = await supabase.rpc('pm_lookup_by_token', { p_token: token });
        ...
        // 2b) Fallback to views (RLS-friendly if you already use them)
        if (!payload) {
        ...
        const q = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(token)}`;
        setQrUrl(q);
      } finally {
        setLoading(false);
      }
  ...
  const onPrint = () => {
    if (!data) return;
    const html = `
      <html><head><title>${hrCode || 'Work Order'} — Print</title>
      <style>
        @page { margin: 14mm; }
      ...
      <div class="header">
        ...
        ${qrUrl ? `<img src="${qrUrl}" style="height:100px;width:100px;border:1px solid #e5e7eb;border-radius:8px;padding:6px;background:#fff"/>` : ''}
      </div>
      <div class="grid">
        ...
        ${data.done_by ? `<div class="card"><div class="label">Done By</div><div class="value">${data.done_by} ${data.done_at ? (' @ ' + new Date(data.done_at).toLocaleString()) : ''}</div></div>` : ''}
        ${data.verified_by ? `<div class="card"><div class="label">Verified By</div><div class="value">${data.verified_by} ${data.verified_at ? (' @ ' + new Date(data.verified_at).toLocaleString()) : ''}</div></div>` : ''}
      </div>
        ...
        <div className="rounded-2xl border shadow-sm bg-white/85 overflow-hidden">
          <div className="px-4 sm:px-6 py-4 flex items-center justify-between border-b bg-slate-50">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={!woToken}
              >
                <ClipboardCopy size={16}/> Copy token
              </button>
              <button
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border bg-white hover:bg-slate-50"
                onClick={onPrint}
          ...
          <div className="p-4 sm:p-6">
            {loading ? (
              ...
              <div className="text-sm text-rose-600">No data found for this Work Order.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl border bg-white p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">Asset</div>
                  <div className="text-[15px] mt-1 font-semibold">{data.asset?.name || '—'}</div>
                  <div className="text-xs text-slate-500">{data.asset?.code || '—'}</div>
                </div>

                <div className="rounded-xl border bg-white p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">Template</div>
                  <div className="text-[15px] mt-1 font-semibold">{data.template || '—'}</div>
                  <div className="text-xs text-slate-500">Freq: {data.freq || '—'}</div>
                </div>

                <div className="rounded-xl border bg-white p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">Scheduled</div>
                  <div className="text-[15px] mt-1 font-semibold">{data.scheduled || '—'}</div>
                </div>

                <div className="rounded-xl border bg-white p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">Status</div>
                  <div className="text-[15px] mt-1 font-semibold">{data.status || '—'}</div>
                </div>

                {data.sop && (
                  <div className="md:col-span-2 rounded-xl border bg-white p-4">
                    <div className="text-[11px] uppercase tracking-wide text-slate-500">SOP / Doc URL</div>
                    <div className="text-[14px] mt-1 break-words">
                      <a href={data.sop} target="_blank" rel="noreferrer" className="text-blue-700 underline">
                        {data.sop}
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="text-[14px] mt-1">
                    {data.done_by ? (
                      <span className="inline-flex items-center gap-1">
                        ...
                        {data.done_by} <span className="text-slate-500">• {fmt(data.done_at)}</span>
                      </span>
                    ) : '—'}
                  </div>
                </div>
                ...
                <div className="rounded-xl border bg-white p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">Verified</div>
                  <div className="text-[14px] mt-1">
                    {data.verified_by ? (
                      <span className="inline-flex items-center gap-1">
                        <ShieldCheck size={16} className="text-blue-700" />
                        {data.verified_by} <span className="text-slate-500">• {fmt(data.verified_at)}</span>
                      </span>
                    ) : '—'}
                  </div>
                </div>
                ...
                <div className="md:col-span-2 rounded-xl border bg-slate-50 p-4">
                  <div className="text-[11px] uppercase tracking-wide text-slate-500 mb-1">QR Token</div>
                  <div className="flex items-center gap-4">
                    {qrUrl && <img src={qrUrl} alt="QR" className="h-28 w-28 rounded border bg-white p-2" />}
                    <div className="text-xs text-slate-600">
                      This QR encodes only the Work Order token (UUID). Your scanner can use this page or the global scan in the sidebar.
                    </div>
                  </div>
                </div>
