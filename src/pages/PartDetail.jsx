...
const PartDetail = () => {
  ...
  const title = useMemo(
    () => part?.part_name || part?.part_code || "Part",
    [part]
  );
  ...
  const load = async () => {
    ...
    try {
      ...
      const pl = await supabase
        .from("part_location")
        .select(
          "id,plant_uid,bin_code,min_qty,max_qty,qty_on_hand,reorder_point,supplier_name,lead_time_days"
        )
        .eq("part_uid", pid);
      ...
      .promise(
        (async () => {
          if (!form.part_code.trim() || !form.part_name.trim()) {
            throw new Error("Part Code & Name required");
          }
          ...
          for (const row of locRows) {
            ...
            const payloadPL = {
              ...
              lead_time_days:
                row.lead_time_days === "" ? null : Number(row.lead_time_days),
            };
  ...
  const onPrintPartLabel = async () => {
    try {
      ...
      const defaultPlant = locRows[0]
        ? plantIdByUid[locRows[0].plant_uid] ||
          locRows[0].plant_id ||
          "Plant1"
        : "Plant1";
...
<head>
  ...
  <style>
    ...
    .card{
      width:${w}mm;height:${h}mm;border:1px solid #cbd5e1;border-radius:6px;
    ...
    .qr img{max-height:16mm;max-width:100%}
...
<body>
  <div class="card">
    <div class="hdr">
      ...
      <div class="title">${esc(form.part_name)}</div>
    </div>
    <div class="sub">ID: ${esc(form.part_code)}</div>
    <div class="kv">UOM: ${
      uoms.find((u) => u.id === form.uom_uid)?.uom_code || "-"
    } • Bin: ${defaultBin || "-"}</div>
    <div class="qr">
      <img src="${qr}" alt="QR"/>
      <div class="cap">QR • ${
        String(part.id).slice(0, 4) + "…" + String(part.id).slice(-4)
      }</div>
    </div>
  </div>
</body>
  ...
  const onDelete = async () => {
    const ok = window.confirm(
      `Delete part "${form.part_code}"? This also removes its locations.`
    );
    if (!ok) return;
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setForm((f) => ({
                            ...
                            uom_uid: part.uom_uid,
                            active: part.active,
                            is_quarantine: part.is_quarantine,
                          }));
                        }}
                      >
                        <X size={16} />
              ...
              <>
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          is_quarantine: e.target.checked,
                        })
                      }
                    />
                    Quarantine
                ...
                <div className="mt-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded border bg-white hover:bg-slate-50 text-sm"
                      onClick={addRow}
                    >
                      <Plus size={16} />
                      Add Row
                            ...
                            <td className="p-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setLocRows((prev) => {
                                    const n = [...prev];
                                    n[idx].reorder_point = e.target.value;
                                    return n;
                                  })
                                }
                              />
                            ...
                            <td className="p-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setLocRows((prev) => {
                                    const n = [...prev];
                                    n[idx].qty_on_hand = e.target.value;
                                    return n;
                                  })
                                }
                              />
                            ...
                            <td className="p-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setLocRows((prev) => {
                                    const n = [...prev];
                                    n[idx].supplier_name = e.target.value;
                                    return n;
                                  })
                                }
                              />
                            ...
                            <td className="p-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setLocRows((prev) => {
                                    const n = [...prev];
                                    n[idx].lead_time_days = e.target.value;
                                    return n;
                                  })
                                }
                              />
