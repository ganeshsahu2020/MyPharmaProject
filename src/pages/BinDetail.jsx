...
import { Printer, Save, Package, QrCode, Warehouse } from 'lucide-react';
...
const BinDetail = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      // join manually: get all locations in this bin, then fetch part names in one go
      const pl = await supabase.from('part_location')
        .select('id,part_uid,qty_on_hand,reorder_point,min_qty,max_qty')
        .eq('plant_uid', p.data.id).eq('bin_code', bin_code);

      const partIds = [...new Set((pl.data || []).map(r => r.part_uid))];
      let namesById = {};
      if (partIds.length) {
        const pm = await supabase.from('part_master').select('id,part_code,part_name').in('id', partIds);
        (pm.data || []).forEach(x => { namesById[x.id] = x; });
      }

      setRows((pl.data || []).map(r => ({
        ...r,
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [plant_id, bin_code]);

  const save = async () => {
    setSaving(true);
    await toast.promise((async () => {
      for (const r of rows) {
        const { error } = await supabase.from('part_location')
          .update({
  ...
  const onPrintBinLabel = async () => {
    ...
    const qr = await makeQR(JSON.stringify({ type: 'bin', plant_id, bin_code }));
    const w = 50, h = 38;
    const html = `<!doctype html><html><head><meta charset="utf-8"/><title>Bin Label</title>
      <style>
        ...
        <div class="card">
          ...
          <div class="qr"><img src="${qr}" alt="QR"/><div class="cap">QR • ${(bin_code.length>8?bin_code.slice(0,4)+'…'+bin_code.slice(-4):bin_code)}</div></div>
        </div>
      </body></html>`;
        ...
        <div className="rounded-2xl border shadow-sm bg-white/80 overflow-hidden">
          <div className="p-4 border-b bg-gradient-to-r from-[#ecf2ff] via-[#f2f7ff] to-[#eefcf6] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-[#eef2ff] text-[#143C8B] flex items-center justify-center ring-1"><Warehouse size={18}/></div>
              <div>
                <div className="text-xl font-extrabold text-[#143C8B]">Bin — {plant_id}/{bin_code}</div>
                <div className="text-xs text-slate-600">{loading ? '' : `${rows.length} part(s)`}</div>
              </div>
            </div>
            <div className="flex gap-2">
              <button className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border bg-white hover:bg-slate-50" onClick={onPrintBinLabel}><Printer size={16}/> Print Label</button>
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="p-2"><input className="border rounded p-1 w-20" type="number" value={r.min_qty} onChange={(e)=>setRows(prev=>prev.map(x=>x.id===r.id?{...x,min_qty:e.target.value}:x))}/></td>
                      <td className="p-2"><input className="border rounded p-1 w-20" type="number" value={r.max_qty} onChange={(e)=>setRows(prev=>prev.map(x=>x.id===r.id?{...x,max_qty:e.target.value}:x))}/></td>
                      <td className="p-2"><input className="border rounded p-1 w-24" type="number" value={r.reorder_point} onChange={(e)=>setRows(prev=>prev.map(x=>x.id===r.id?{...x,reorder_point:e.target.value}:x))}/></td>
                      <td className="p-2"><input className="border rounded p-1 w-24" type="number" value={r.qty_on_hand} onChange={(e)=>setRows(prev=>prev.map(x=>x.id===r.id?{...x,qty_on_hand:e.target.value}:x))}/></td>
                    </tr>
                  ))}
                  {!rows.length && <tr><td className="p-3 text-slate-500" colSpan={6}>No parts in this bin.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
